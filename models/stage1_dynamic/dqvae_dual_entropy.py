...
from modules.dynamic_modules.utils import (
    draw_dual_grain_256res,
    draw_dual_grain_256res_color,
)
from models.stage1.utils import (
...
class Entropy(nn.Sequential):
    def __init__(self, patch_size, image_width, image_height):
        ...
        self.unfold = torch.nn.Unfold(
            kernel_size=(self.psize, self.psize), stride=self.psize
        )

    def entropy(
        self, values: torch.Tensor, bins: torch.Tensor, sigma: torch.Tensor, batch: int
    ) -> torch.Tensor:
    ...
    def forward(self, inputs: Tensor) -> torch.Tensor:
        ...
        unfolded_images = torch.reshape(
            unfolded_images.unsqueeze(2),
            (unfolded_images.shape[0] * self.patch_num, unfolded_images.shape[2]),
        )

        entropy = self.entropy(
            unfolded_images,
            bins=torch.linspace(-1, 1, 32).to(device=inputs.device),
