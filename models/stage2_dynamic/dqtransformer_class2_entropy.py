...
from models.stage2.utils import (
...
class Dualformer(pl.LightningModule):
    def __init__(
        ...
        self.activate_segment = (
            True if transformer_config["params"]["segment_size"] > 0 else False
        )
        ...
        self.coarse_position_eos_code = permuter_config["params"][
            "coarse_position_eos_code"
        ]
        self.coarse_position_pad_code = permuter_config["params"][
            "coarse_position_pad_code"
        ]
        # self.fine_position_sos_code = class_cond_stage_config["params"]["fine_pos_sos"] #
        self.fine_position_eos_code = permuter_config["params"][
            "fine_position_eos_code"
        ]
        self.fine_position_pad_code = permuter_config["params"][
            "fine_position_pad_code"
        ]
    ...
    def decode_to_img(
        ...
        reproduced_indices = self.permuter.forward_back(
            coarse_content, fine_content, coarse_position, fine_position
        )
        reproduced_quant = self.first_stage_model.get_code_emb_with_depth(
            reproduced_indices
        )
        reproduced_rec = self.first_stage_model.decode(
            reproduced_quant.permute(0, 3, 1, 2)
        )
    ...
    def forward(self, x, c):
        ...
        output = self.transformer(
            coarse_content=az_coarse_content,
            fine_content=az_fine_content,
            coarse_position=az_coarse_position,
            fine_position=az_fine_position,
            coarse_seg=az_coarse_segment,
            fine_seg=az_fine_segment,
            content_target=content_target,
            coarse_position_target=coarse_position_target,
            fine_position_target=fine_position_target,
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train_coarse_position_loss",
            output["coarse_position_loss"],
            prog_bar=False,
            logger=True,
            on_step=True,
            on_epoch=True,
        ...
        self.log(
            "train_fine_position_loss",
            output["fine_position_loss"],
            prog_bar=False,
            logger=True,
            on_step=True,
            on_epoch=True,
    ...
    def log_images(
        ...
        **kwargs,
        ...
        log["samples_fixed_fine_position"] = self.decode_to_img(
            coarse_content, fine_content, coarse_position, fine_position
        )
        # end_time = time.time()
        ...
        log["samples_from_scratch"] = self.decode_to_img(
            coarse_content, fine_content, coarse_position, fine_position
        )
        ...
        if self.current_epoch == 0:
            # reconstruction
            _, z_out = self.encode_to_z(x)
            x_rec = self.decode_to_img(
                z_out["coarse_content"],
                z_out["fine_content"],
                z_out["coarse_position"],
                z_out["fine_position"],
            )
    ...
    def sample_from_scratch(
        ...
        if self.activate_sos_for_fine_sequence:
            x_coarse, x_fine, x_pos_coarse, x_pos_fine, x_seg_coarse, x_seg_fine = (
                c_coarse,
                c_fine,
                c_pos_coarse,
                c_pos_fine,
        ...
        while not torch.all(
            sample_coarse_flag.bool()
        ):  # sample coarse grain code, torch.all(sample_coarse_flag) is True only if all elements are True
            # sample position first
            position_hidden, position_logits = self.transformer.sample_coarse_position(
                coarse_content=x_coarse,
                coarse_position=x_pos_coarse,
                coarse_seg=x_seg_coarse,
            )
            position_logits = position_logits[:, -1, :] / temperature
            position_logits = self.avoid_repeat_or_enforce_pad_for_coarse_position(
                position_logits, x_pos_coarse, sample_coarse_flag
            )
            ...
            sample_coarse_flag = sample_coarse_flag + (
                ix_pos == self.coarse_position_eos_code
            )

            # sample content next
            _, content_logits = self.transformer.sample_coarse_content(
                coarse_content=None,
                coarse_position=x_pos_coarse,
                coarse_seg=None,
            ...
            content_logits = self.avoid_special_or_enforce_pad_for_content(
                content_logits, sample_coarse_flag
            )
            ...
            if self.activate_segment:
                x_seg_coarse = torch.cat(
                    [x_seg_coarse, torch.zeros(batch_size, 1).long().to(device)], dim=1
                )
            else:
                pass
        ...
        if not fix_fine_position:  # sample with sampled fine position
            transfered_fine_position = (
                self.transfer_sampled_coarse_position_to_sampled_fine_position(
                    x_pos_coarse
                )
            )
            ...
            while not torch.all(
                sample_fine_flag.bool()
            ):  # sample coarse grain code, torch.all(sample_coarse_flag) is True only if all elements are True
                position_hidden, position_logits = (
                    self.transformer.sample_fine_position(
                        coarse_content=x_coarse,
                        fine_content=x_fine,
                        coarse_position=x_pos_coarse,
                        fine_position=x_pos_fine,
                        coarse_seg=x_seg_coarse,
                        fine_seg=x_seg_fine,
                    )
                )
                position_logits = position_logits[:, -1, :] / temperature
                position_logits = self.avoid_repeat_or_enforce_pad_for_fine_position(
                    position_logits, transfered_fine_position, sample_fine_flag
                )
                if top_k_pos is not None:
                    position_logits = top_k_logits(position_logits, top_k_pos)
                # apply softmax to convert to probabilities
                ...
                transfered_fine_position = torch.cat(
                    [transfered_fine_position, ix_pos], dim=1
                )

                # 如果某一个样本sample到了<eos> code，则该样本后续均为<pad> code
                sample_fine_flag = sample_fine_flag + (
                    ix_pos == self.fine_position_eos_code
                )

                # sample content next
                _, content_logits = self.transformer.sample_fine_content(
                    coarse_content=x_coarse,
                    fine_content=x_fine,
                    coarse_position=x_pos_coarse,
                    fine_position=x_pos_fine,
                ...
                content_logits = content_logits[:, -1, :] / temperature
                content_logits = self.avoid_special_or_enforce_pad_for_content(
                    content_logits, sample_fine_flag
                )
                if top_k is not None:
                    content_logits = top_k_logits(content_logits, top_k)
                # apply softmax to convert to probabilities
                ...
                if self.activate_segment:
                    x_seg_fine = torch.cat(
                        [x_seg_fine, torch.ones(batch_size, 1).long().to(device)], dim=1
                    )
                else:
                    pass
                ...
                if process:
                    print(
                        "\r sampled coarse size: {} ; sampling fine: {}".format(
                            x_coarse.size(1), x_fine.size(1)
                        ),
                        end="",
                    )
        else:
            transfered_remain_fine_position = (
                self.transfer_sampled_coarse_position_to_remain_fine_position(
                    x_pos_coarse
                )
            )
            sample_fine_flag = torch.zeros(batch_size, 1).to(device)
    ...
    def transfer_sampled_coarse_position_to_remain_fine_position(self, coarse_position):
        ...
        grain_indices = rearrange(
            grain_indices, "B (h w) -> B h w", h=self.hw1, w=self.hw1
        )
        ...
        if self.fine_position_order == "region-first":
            ...
            grain_indices = grain_indices.repeat_interleave(
                2, dim=-1
            ).repeat_interleave(2, dim=-2)
            fine_position_list = [
                torch.cat(
                    [
                        self.position_sequence_fine[grain_indices[i] == 1].to(device),
                        self.fine_position_eos_tensor.to(device),
                    ]
                )
                for i in range(batch_size)
    ...
    def transfer_sampled_coarse_position_to_sampled_fine_position(
        ...
        for i in range(batch_size):
            for coarse_l in range(coarse_length):
                if coarse_position[i, coarse_l] == self.coarse_position_eos_code:
                    ...
                    grain_indices[i, coarse_position[i, coarse_l]] = (
                        1  # means this coarse-position is sampled and will transfer to fine position later
                    )
    ...
    def avoid_repeat_or_enforce_pad_for_coarse_position(
        self, logits, sampled_position, flag
    ):
        ...
        for i in range(batch_size):
            if (
                flag[i] == 0
            ):  # avoid sample <sos>, <pad>, finer position and sampled coarse position
                out[i, sampled_position[i]] = -float(
                    "Inf"
                )  # forbid <sos> and sampled coarse position
                out[i, self.coarse_position_pad_code] = -float("Inf")  # forbid <pad>
                out[i, self.max_coarse_postion_idx :] = -float(
                    "Inf"
                )  # forbid finer position
                out[i, self.coarse_position_eos_code] = logits[
                    i, self.coarse_position_eos_code
                ]
            else:  # enforce sample <pad>
                out[i, :] = -float("Inf")
                out[i, self.coarse_position_pad_code] = logits[
                    i, self.coarse_position_pad_code
                ]
        return out

    def avoid_repeat_or_enforce_pad_for_fine_position(
        self, logits, sampled_position, flag
    ):
        batch_size = logits.size(0)
        out = logits.clone()
        for i in range(batch_size):
            if flag[i] == 0:  # avoid repeat and <sos>, <pad>
                ...
                out[i, self.fine_position_eos_code] = logits[
                    i, self.fine_position_eos_code
                ]  # keep <eos>
                # out[i, self.fine_position_sos_code] = -float('Inf')
                out[i, self.fine_position_eos_code + 1 :] = -float(
                    "Inf"
                )  # NOT including <eos>, avoid class-label
            else:
                out[i, :] = -float("Inf")
                out[i, self.fine_position_pad_code] = logits[
                    i, self.fine_position_pad_code
                ]
        return out
    ...
    def avoid_special_or_enforce_pad_for_content(self, logits, flag):
        ...
        for i in range(batch_size):
            if flag[i] == 0:  # avoid special
                ...
                out[i, self.content_eos_code :] = -float(
                    "Inf"
                )  # avoid class-label sampling, including <eos>
