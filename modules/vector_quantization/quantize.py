...
class VectorQuantize(nn.Module):
    ...
    def forward(self, x, temp=0.0):
        ...
        if self.use_cosine_sim:  # cosine similarity
            ...
            dist = (
                -torch.sum(flatten_norm**2, dim=1, keepdim=True)
                - torch.sum(weight_norm**2, dim=1)
                + 2
                * torch.einsum(
                    "bd,dn->bn", flatten_norm, rearrange(weight_norm, "n d -> d n")
                )
            )  # more efficient, add "-" for argmax gumbel sample
        else:  # L2 distance
            ...
            dist = (
                ...
                + 2
                * torch.einsum(
                    "bd,dn->bn", flatten, rearrange(self.embedding.weight, "n d -> d n")
                )
            )  # more efficient, add "-" for argmax gumbel sample
        ...
        if self.orthogonal_reg_weight > 0.0:
            ...
            diff = torch.mm(
                emb_weight_after_norm, torch.transpose(emb_weight_after_norm, 0, 1)
            ) - torch.eye(self.codebook_size, self.codebook_size).type_as(
                emb_weight_after_norm
            )
            ortho_reg_term = (
                self.orthogonal_reg_weight * torch.sum(diff**2) / (diff.size(0) ** 2)
            )
...
if __name__ == "__main__":
    ...
    x_q, loss, (_, _, embed_ind) = quantizer(x, 0.0)
    print(x_q.shape, loss, (_, _, embed_ind.shape))
